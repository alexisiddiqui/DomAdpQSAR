...
from DomAdpQSAR.utility import MixtureModel
...
def generate_display_frame(fake_examples, unlabeled_predictions, test_predictions, dnn_test_predictions,
    ...
    try:
        axes = sns.kdeplot(fake_a3[0, :], ax=axes, color=sns.color_palette()[4], bw_method=bandwidth,
    ...
    axes = sns.kdeplot(dnn_test_predictions, ax=axes, color=sns.color_palette()[3], bw_method=bandwidth,
                       label='DNN Test Predictions')
    axes = sns.kdeplot(dnn_train_predictions, ax=axes, color=sns.color_palette()[3], linewidth=0.5, bw_method=bandwidth,
